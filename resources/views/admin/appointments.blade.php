...
<div class="max-w-7xl mx-auto px-2 sm:px-4 lg:px-6 xl:px-8 py-4 sm:py-8">
    ...
    <div class="mb-6 sm:mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Appointment Requests</h1>
                <p class="mt-1 sm:mt-2 text-sm sm:text-base text-gray-600">Review and manage incoming appointment requests</p>
            </div>
            <div class="flex items-center gap-2 px-3 sm:px-4 py-2 bg-blue-50 border border-blue-200 rounded-lg">
                <span class="text-xs sm:text-sm font-medium text-blue-900">Total:</span>
                <span class="text-xl sm:text-2xl font-bold text-blue-600">{{ $appointments->total() }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-4 sm:p-6 shadow-sm mb-8">
        <form method="GET" action="{{ route('admin.appointments') }}" class="space-y-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4">
                ...
                <!-- Archive Filter -->
                <div>
                    <label for="archive" class="block text-sm font-medium text-gray-700 mb-2">Archive</label>
                    <select id="archive" 
                            name="archive"
                            class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition">
            ...
            <details class="border-t border-gray-200 pt-4">
                <summary class="cursor-pointer font-medium text-gray-700 hover:text-gray-900">Date Range Filter</summary>
    ...
    <div class="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
                ...
                <table class="w-full min-w-max sm:min-w-full">
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr>
                            ...
                            <th class="hidden sm:table-cell px-4 sm:px-6 py-4 text-left text-sm font-semibold text-gray-900">Date</th>
                            <th class="hidden md:table-cell px-4 sm:px-6 py-4 text-left text-sm font-semibold text-gray-900">Purpose</th>
                            <th class="px-4 sm:px-6 py-4 text-left text-sm font-semibold text-gray-900">Status</th>
                            <th class="hidden lg:table-cell px-4 sm:px-6 py-4 text-left text-sm font-semibold text-gray-900">Requested</th>
                        ...
                        @foreach($appointments as $appointment)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="hidden sm:table-cell px-4 sm:px-6 py-4">
                                    ...
                                    <p class="text-xs text-gray-500">{{ $appointment->availability->date->format('l') }}</p>
                                </td>
                                <td class="hidden md:table-cell px-4 sm:px-6 py-4">
                                    <span class="inline-block px-3 py-1 bg-blue-50 border border-blue-200 text-blue-700 text-xs sm:text-sm font-medium rounded-full">
                                        {{ $appointment->purpose }}
                                    </span>
                                </td>
                                <td class="px-4 sm:px-6 py-4">
                                    @if($appointment->status === 'pending')
                                        <span class="inline-flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1 bg-yellow-50 border border-yellow-200 text-yellow-700 text-xs sm:text-sm font-medium rounded-full">
                                            <i class="fi fi-rr-hourglass text-xs hidden sm:inline"></i>
                                    ...
                                    @elseif($appointment->status === 'approved')
                                        <span class="inline-flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1 bg-green-50 border border-green-200 text-green-700 text-xs sm:text-sm font-medium rounded-full">
                                            <i class="fi fi-rr-check text-xs hidden sm:inline"></i>
                                            <span>Approved</span>
                                        </span>
                                    @else
                                        <span class="inline-flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1 bg-red-50 border border-red-200 text-red-700 text-xs sm:text-sm font-medium rounded-full">
                                            <i class="fi fi-rr-cross text-xs hidden sm:inline"></i>
                                            <span>Rejected</span>
                                        </span>
                                    @endif
                                </td>
                                    ...
                                    <div class="flex items-center gap-1 sm:gap-2 flex-wrap justify-end sm:justify-start">
                                        @if($appointment->status === 'pending')
                                            <button onclick="window.approveAppointment({{ $appointment->id }}, '{{ addslashes($appointment->user->profile->fname ?? 'User') }} {{ addslashes($appointment->user->profile->lname ?? '') }}')"
                                                    class="px-2 sm:px-3 py-1 bg-green-50 text-green-700 border border-green-200 text-xs sm:text-sm font-medium rounded-lg hover:bg-green-100 transition active:scale-95 whitespace-nowrap">
                                                <i class="fi fi-rr-check mr-0 sm:mr-1"></i><span class="hidden sm:inline">Approve</span>
                                            </button>
                                            <button onclick="window.rejectAppointment({{ $appointment->id }}, '{{ addslashes($appointment->user->profile->fname ?? 'User') }} {{ addslashes($appointment->user->profile->lname ?? '') }}')"
                                                ...
                                                <i class="fi fi-rr-cross mr-0 sm:mr-1"></i><span class="hidden sm:inline">Reject</span>
                                            </button>
                                        @else
                                            <button onclick="window.archiveAppointment({{ $appointment->id }}, '{{ addslashes($appointment->user->profile->fname ?? 'User') }} {{ addslashes($appointment->user->profile->lname ?? '') }}')"
                                                    class="px-2 sm:px-3 py-1 bg-purple-50 text-purple-700 border border-purple-200 text-xs sm:text-sm font-medium rounded-lg hover:bg-purple-100 transition active:scale-95 whitespace-nowrap">
                                                <i class="fi fi-rr-box mr-0 sm:mr-1"></i><span class="hidden sm:inline">Archive</span>
                                            </button>
        ...
        @else
            <div class="px-4 sm:px-6 py-12 text-center">
                <div class="mb-4">
                    <i class="fi fi-rr-calendar text-6xl text-gray-300"></i>
                </div>
                <h3 class="text-lg font-medium text-gray-900 mb-2">No Appointments Found</h3>
                <p class="text-gray-600">There are no appointment requests matching your filters.</p>
            </div>
        @endif
    </div>
</div>
